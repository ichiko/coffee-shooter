(function() {
  var ArcSprite, CrushParticle, ENEMY_INIT_POS_RANGE, ENEMY_INIT_POS_TOP, ENEMY_INIT_POS_X, ENEMY_POP_DELAY, Enemy, FPS, IMG_ENEMY, IMG_MUSHROOM, IMG_PLAYER, MUSH_INIT_POS_Y, Mush, PLAYER_INIT_POS_X, PLAYER_INIT_POS_Y, Player, ResultScene, SCREEN_SIZE, STATE_MAINGAME, STATE_RESULT, ShootScene, Shooter, StateLabel, TIME_LIMIT,
  ...
  ShootScene = (function(_super) {
    ...
    function ShootScene() {
      ...
      this.player = new Player(PLAYER_INIT_POS_X, PLAYER_INIT_POS_Y);
      ...
      this.onenter = function() {
        this.initScene();
        this.player.tl.clear();
        return this.tick = 0;
      };
      this.ontouchstart = function(e) {
        var _this = this;
        return this.player.tl.moveTo(e.x, this.player.y, Math.abs(this.player.x - e.x) / 4).then(function() {
          return _this.mainGroup.addChild(new Mush(e.x, MUSH_INIT_POS_Y));
        });
      };
    ...
    ShootScene.prototype.initScene = function() {
      this.player.x = PLAYER_INIT_POS_X;
      if (this.mainGroup) {
  ...
  Mush = (function(_super) {
    ...
    Mush.prototype.onenterframe = function() {
      ...
      while (i < len) {
        ...
        if (elm !== this && elm instanceof Enemy && elm.intersect(this) === true) {
          this.game.incrementScore(100);
          this.parentNode.addChild(new CrushParticle(elm.x, elm.y));
  ...
  ArcSprite = (function(_super) {
    __extends(ArcSprite, _super);

    function ArcSprite(radius) {
      var c, surface;
      ArcSprite.__super__.constructor.call(this, radius * 2, radius * 2);
  ...
  CrushParticle = (function(_super) {
    __extends(CrushParticle, _super);

    function CrushParticle(x, y) {
      var i, particle, radius, _i;
      CrushParticle.__super__.constructor.call(this);
      this.x = x;
      this.y = y;
    ...
    CrushParticle.prototype.onenterframe = function() {
      var elm, i, len;
      i = 0;
      len = this.childNodes.length;
      while (i < len) {
        elm = this.childNodes[i];
        if (typeof elm !== "undefined" && elm.update() === false) {
  ...
  ResultScene = (function(_super) {
    ...
    ResultScene.prototype.setEventDelay = function() {
      ...
      btn.ontouchend = function() {
        return this.tl.delay(this.game.fps * 0.1).then(function() {
          this.game.popScene();
